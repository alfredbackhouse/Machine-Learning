...
let gen_addr d =
  if d.d_level = 0 then
    ...
    SEQ [LOCAL 0; SEQ (List.init (!level - d.d_level) (fun _ ->SEQ [CONST 12; OFFSET; LOADW])); CONST d.d_offset;OFFSET]
     
let gen_staticlink d =
  if d.d_level = 0 then CONST 0
  else SEQ [LOCAL 0;SEQ (List.init (!level - d.d_level + 1) (fun _ ->SEQ [CONST 12; OFFSET; LOADW]))]
...
let rec gen_expr =
  function
      Variable x ->
        ...
        begin
          match d.d_kind with
            ...
            | ProcDef nargs -> SEQ [LINE x.x_line;gen_staticlink d;GLOBAL d.d_label; PACK]
        end
    | Constant x ->
        CONST x
    | Monop (w, e1) ->
    ...
    | Call (p, args) ->
    	let arguements = List.rev(List.map gen_expr args) in 
    	let d = get_def p in
        SEQ (List.append   (LINE p.x_line::arguements)    [gen_expr (Variable p); UNPACK; PCALLW (List.length args)])
